...
from reportlab.platypus import (
    SimpleDocTemplate, Paragraph, Spacer, HRFlowable, ListFlowable, ListItem, KeepTogether
)
...
# CONFIG
# -------------------------------------------------
ACCESS_CODE = "PHONE2026"          # Change this to whatever you want
...
# -------------------------------------------------
st.set_page_config(
    page_title="Phone to Paycheck | Personalized Business Blueprint",
    page_icon="📱",
    layout="centered",
...
<style>
    ...
    .big-font { font-size: 1.25rem !important; }
    .plan-box {
...
# -------------------------------------------------
# Decision + Content Generation Logic
# -------------------------------------------------
def generate_blueprint(answers):
    ...
    if primary == "freelance":
        ...
        tools = [
            "Fiverr app – create seller account",
            "Canva – make portfolio samples & gig images",
            "Gmail – clean professional email",
            "Google Drive – store samples & deliveries"
        ]
        day1_7 = [
            ...
            "Day 3: Create your first highly specific gig (example: “I will write 10 Instagram captions for " + niche + " brands”).",
        ...
        days_8_30 = [
            ...
            "Days 22–30: Create a simple portfolio page (Canva or Google Site). Start offering the same services via Stripe Payment Links for direct clients so you keep more of the money."
        ]
        money = "Fiverr handles all payments (they take ~20%). Once you have reviews, raise your prices. For higher-margin direct clients later, create free Stripe Payment Links (see instructions below)."
        ...
        ideas = [
            ...
            f"Design simple Canva templates for {niche} (posts, stories, or planners)",
        ...
        tools = [
            "CapCut – free powerful video editor",
            "TikTok + Instagram + YouTube apps",
            "Canva – thumbnails, graphics, end screens",
            "Later: Gumroad or Payhip for your own products"
        ]
        day1_7 = [
            "Day 1: Download CapCut, TikTok, Instagram, YouTube. Create clean profiles with a clear bio.",
            "Day 2: Choose your angle inside “" + niche + "”. Study 8–10 top accounts and note their hooks.",
        ...
        days_8_30 = [
            ...
            "Days 22–30: Create your first simple digital product or affiliate offer and promote it from your best videos. Set up Gumroad/Payhip or Stripe Payment Links."
        ]
        ...
        tools = [
            "Canva – design the actual products + mockups",
            "Gumroad or Payhip – storefront + automatic delivery + payments",
            "CapCut – optional short promo videos",
            "TikTok / Instagram – free traffic"
        ]
        day1_7 = [
            "Day 1: Download Canva. Create free accounts on Gumroad and/or Payhip.",
            "Day 2: Design your first simple digital product related to “" + niche + "” (planner, checklist, template pack, mini-guide, etc.).",
        ...
        tools = [
            "CapCut – edit your samples and client videos",
            "Google Drive or Canva – simple portfolio",
            "Billo + JoinBrands – beginner-friendly UGC platforms",
            "Phone with good natural light"
        ]
        day1_7 = [
...
def create_pdf(plan):
    ...
    story.append(Paragraph("Stripe Payment Links (Advanced / Higher Margin)", styles["SectionHead"]))
    story.append(Paragraph(
        "1. Go to stripe.com and create a free account (works on mobile browser).<br/>"
        "2. In the Dashboard go to Payment Links → Create payment link.<br/>"
        "3. Add a title, price, and optional description/image.<br/>"
        "4. Copy the link and share it anywhere (bio, DMs, Fiverr custom offers, etc.).<br/>"
        "5. Stripe handles the payment and deposits money to your bank. You only pay normal Stripe processing fees (no big platform cut).",
        styles["BodyText2"]
    ))
...
def show_login():
    ...
    st.write("Enter the access code you received after purchase to unlock your interactive guide.")
    ...
    if st.button("Unlock →", type="primary"):
        if code.strip().upper() == ACCESS_CODE.upper():
            ...
            st.error("Incorrect code. Please check the email or message you received after purchase.")

def show_welcome():
    st.title("📱 Phone to Paycheck")
    st.subheader("Your Personalized Business Blueprint")
    st.success("Access granted. Let’s build your custom plan.")
    st.write("Answer a few quick questions (about 2 minutes). The more honest you are, the better and more specific your blueprint will be.")
...
def show_q_time():
    ...
    time = st.radio("How much time can you realistically put in most days?",
                    ["15–30 minutes", "30–60 minutes", "1–2 hours", "2+ hours"], index=1)
    c1, c2 = st.columns(2)
    with c1:
        if st.button("← Back"):
            st.session_state.step = 0
...
def show_q_pref():
    ...
    pref = st.radio("Choose the option that feels easiest or most exciting:",
                    ["Talking on camera", "Writing & designing", "Helping people with tasks",
                     "Creating something once and selling repeatedly", "Not sure — recommend the best for me"])
    c1, c2 = st.columns(2)
    with c1:
        if st.button("← Back"):
            st.session_state.step = 1
...
def show_q_skills():
    ...
    skills = st.multiselect("Select all that apply:",
                            ["Social media following", "Writing skill", "Design / Canva experience",
                             "Comfort talking on video", "None yet"])
    c1, c2 = st.columns(2)
    with c1:
        if st.button("← Back"):
            st.session_state.step = 2
...
def show_q_goal():
    ...
    goal = st.radio("What do you most want right now?",
                    ["First $100 as fast as possible", "Consistent $300–500 / month side income",
                     "Reach $1,000+ / month", "Build toward replacing a job"])
    c1, c2 = st.columns(2)
    with c1:
        if st.button("← Back"):
            st.session_state.step = 3
...
def show_q_niche():
    ...
    st.write("What topics do you already know or enjoy? This lets me give you concrete content and product ideas.")
    niche = st.text_input("Examples: money tips, coffee, fitness, skincare, phone photography, parenting, books…",
                          placeholder="Type a few words or leave blank")
    c1, c2 = st.columns(2)
    with c1:
        if st.button("← Back"):
            st.session_state.step = 4
...
def show_blueprint():
    plan = generate_blueprint(st.session_state.answers)

    st.title("🎉 Your Personalized Blueprint")
    st.caption(f"Generated {datetime.now().strftime('%B %d, %Y')} • Phone to Paycheck")

    st.markdown(f"""
    <div class="plan-box">
        <h3 style="margin-top:0; color:#1A365D;">Recommended Primary Path</h3>
        <p class="big-font"><strong>{plan['primary']}</strong></p>
        <p>{plan['why']}</p>
    </div>
    """, unsafe_allow_html=True)

    if plan["secondary"]:
        st.markdown(f"**Secondary path to add later:** {plan['secondary']}")

    st.markdown(f"**Niche focus:** {plan['niche']}  \n**Time:** {plan['time']}  |  **Goal:** {plan['goal']}")
    ...
    with st.expander("🔑 How to Create Stripe Payment Links (Higher Margin Option)"):
        st.markdown("""
        1. Go to [stripe.com](https://stripe.com) on your phone browser and create a free account.  
        2. In the Stripe Dashboard tap **Payment Links** → **Create payment link**.  
        3. Add a product name, price, and optional image/description.  
        4. Copy the link — you can now share it in your bio, DMs, Fiverr custom offers, emails, etc.  
        5. When someone pays, Stripe deposits the money (minus normal processing fees) straight to your bank.  
        No big marketplace cut. Perfect once you start getting direct clients or want to sell products yourself.
        """)

    st.subheader("🔗 Key Mobile Sites")
    st.markdown("""
    - **Canva** → canva.com or app stores  
    - **CapCut** → App Store / Play Store  
    - **Fiverr** → fiverr.com or app  
    - **Gumroad** → gumroad.com  
    - **Payhip** → payhip.com (often better free-plan fees)  
    - **Stripe Payment Links** → stripe.com/payments/payment-links  
    - **Billo / JoinBrands** → search in browser  
    - **TikTok / Instagram / YouTube** → official apps  
    """)

    # PDF Download
    pdf_buffer = create_pdf(plan)
    st.download_button(
        label="📥 Download My Full Blueprint (PDF)",
        data=pdf_buffer,
        file_name="Phone_to_Paycheck_My_Blueprint.pdf",
        mime="application/pdf",
        type="primary"
    )

    st.success("You now have a clear, personalized map. The only thing left is to take the first step today.")
